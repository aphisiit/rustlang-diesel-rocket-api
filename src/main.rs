...
#[get("/<id>")]
fn getById(id: i32, connection: db::Connection) -> Json<JsonValue> {
    // Json(json!(Hero::getById(id, &connection)))
    let result = Hero::getById(id, &connection);
    Json(json!(
        match result {
            Ok(n)  => n,
            Err(error) => panic!("Problem search with id {:?}: {:?}", id, error)
        }
    ))
}
...
fn main() {
    rocket::ignite().manage(db::connect())
        .mount("/hero", routes![create, update, delete, getById])
        // .mount("/hero", routes![update, delete, getById])
